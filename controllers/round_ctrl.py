...
class RoundController:
    ...
    def start_round(self):
        tournaments = filter(lambda tournament: len(tournament.players) % 2 == 0 and len(tournament.players) >= 8, self.tournaments)
        ...
        if current_tournament is not None:
            if len(current_tournament.rounds) < int(current_tournament.total_rounds):
                if len(current_tournament.rounds) > 0 and current_tournament.rounds[-1].end is None:
                    ...
                    new_round_name = "Round 1" if len(current_tournament.rounds) == 0 else f"Round {len(current_tournament.rounds)+1}"
                    new_round_start = datetime.now()
                    new_round = Round(new_round_name, new_round_start)
                    current_tournament.rounds.append(new_round)
                    generate_players_pairs(current_tournament.players, new_round, current_tournament.players_total_score)
            else:
                print("Tous les rounds de ce tournoi ont été joués.")

    def rounds_results(self):
        tournaments = filter(lambda tournament: len(tournament.rounds) > 0 and tournament.rounds[-1].end is None, self.tournaments)
